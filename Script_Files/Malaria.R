...
my_data <- x|>
  ...
  filter(Sex != "Total" & Disease ==  "Malaria", .by = Year) |>
...
my_graph <- my_data |>
  ...
  labs(
    title = "Malaria Infected Males vs. Females over 2001-2014",
    subtitle = "Males seem to contract Malaria at a much larger rate",
    y = "Infected",
    x = "Years"
  )
#Assigning my_graph to an rds file to make it easier for the respective qmd files to run 
write_rds(my_graph, "Malaria_FM.rds")
